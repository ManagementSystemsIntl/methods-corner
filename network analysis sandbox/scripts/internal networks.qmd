...
# Summary Statistics
We received r nrow(df) responses to the survey with r nrow(df[df$home_field=="Home office",]) responses from home office staff and r nrow(df[df$home_field=="Field office,]) responses from field office staff. 

```{r}

#importing and cleaning column names

df <- here::here(read_xlsx("network analysis sandbox/data/staff_survey_data_03-21.xlsx")) |>
  select(Name 
...
ggplot(data = df2
  ...
  geom_text(aes(x = Software, y = Count, label = Count)
            , color = "white") +
  labs(title = "Most used softwares, per responses"
       , subtitle = "SPSS, Stata, and R were the most commonly used."
       , y = "Number of staff"
       , x = "Software") + 
  theme.plot()
