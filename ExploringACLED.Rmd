...
```{r echo=F, fig.cap="Note how fig captions require 'echo=F' so the reader won't have the option of looking at the code generating the plot"}
...
```{r echo=F, fig.cap="I wish there were a way to add a comma to the numeric label. You might also try using a rectangle geom rather than circle in order to reduce dead space"}
...
```{r fig.dim=c(12,13), echo=F, fig.cap="So much going on here. I wonder if you could have pivoted longer and used a facet wrap. You could also generate a plot for each type and then put them together using the patchwork package"}
...
```{r, fig.dim = c(10,11), echo=F, fig.cap="Marvelous"}
