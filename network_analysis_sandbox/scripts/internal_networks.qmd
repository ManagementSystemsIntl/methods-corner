...
ggplot(data = as.data.frame(gl_tech.apls)) +
  ...
  geom_vline(xintercept = avg_dist_tech
             ...
             , linetype = "dashed"
             , color = my_pal[[14]]) +
  annotate(x = avg_dist_tech + .5, y = 150, label = "Average distance between connections \nin the technical guidance network", geom = "text", color = my_pal[[14]]) +
  annotate(x = avg_dist_tech + .5, y = 100
           , label = glue::glue("{mean(gl_tech.apls < avg_dist_tech) * 100}", "% ", "of similar networks have a shorter \naverage distance between \nconnections than the technical guidance network"), geom = "text", color = my_pal[[11]])+
  labs(y = "# of simulations"
       , x = "Average distance between connections"
       , title = "How does our network compare to similarly structured networks?")+
  theme.plot()
