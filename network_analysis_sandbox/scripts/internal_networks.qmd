...
```{r}
ggplot(data = as.data.frame(gl_tech_avg_dist)) +
  geom_histogram(aes(x = gl_tech_avg_dist), bins = 25
                 , fill = my_pal[[11]]
                 , alpha = .4) +
  ...
  labs(y = "# of simulations"
       , x = "Average distance between connections"
       , title = "How does our network compare to \nsimilarly structured networks?"
       , subtitle = glue::glue("{mean(gl_tech_avg_dist < avg_dist_tech) * 100}", "% ", "of similar networks have a shorter average distance between \nconnections than the technical guidance network"))+
  theme.plot()
