...
Within the SEA practice area we have `r gorder(df_graph_ment)` nodes in our mentorship network consisting of `r gsize(df_graph_ment)` edges. In the technical guidance network we have `r gorder(df_graph_tech)` nodes and `r gsize(df_graph_tech)` edges.
...
g1 <- ggraph(df_graph_ment, layout = "with_kk") +
...
g2 <- ggraph(df_graph_tech, layout = "with_kk") +
