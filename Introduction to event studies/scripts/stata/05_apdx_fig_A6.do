...
/* 2,7+: first two dummies are the same */	
cnsreg y D_m* D_p* 
	ibn.t ibn.i , nocons constraints(2 7 9)  collinear ;
matrix myb_ES1 = e(b) ;
...
/* 2,7+: no pre-trend in ES dummies, first three dums are the same */	
cnsreg y D_m* D_p* 
	ibn.t ibn.i , nocons constraints(2 7 8 9 11)  collinear ;
matrix myb_ES4 = e(b) ;
...
local note1 "gam(-10)=gam(-9)" ;
local note2 "gam(-10)=gam(-9)=gam(-8)" ;
local note3 "gam(-10)=gam(-9)=gam(-8); gam(+10)=gam(+9)=gam(+8)" ;
local note4 "pre-gammas have 0 trend" ;
...
graph combine g1 g2 g3 g4 , ti("T=20, E1=10, E2=11") note("All models: Avg(alpha)=0; Avg(gam(-))=0") ;
graph export figures/apdx_fig_A06.png , replace ;
